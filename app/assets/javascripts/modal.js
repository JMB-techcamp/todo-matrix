$(document).on('turbolinks:load', function(){
 ...
 $("#settings").click(function(){
    ...
    //モーダルウィンドウを表示
        $("#modal-bg,#modal-setting").fadeIn("fast");

    //画面のどこかをクリックしたらモーダルを閉じる
      $("#modal-bg,#modal-setting").click(function(){
          $("#modal-setting,#modal-bg").fadeOut("fast",function(){
         //挿入した<div id="modal-bg"></div>を削除
              $('#modal-bg').remove() ;
         });
    ...
    //画面の左上からmodal-mainの横幅・高さを引き、その値を2で割ると画面中央の位置が計算できます
     $(window).resize(modalResize);
      function modalResize(){
          ...
          var cw = $("#modal-setting").outerWidth();
          var ch = $("#modal-setting").outerHeight();
